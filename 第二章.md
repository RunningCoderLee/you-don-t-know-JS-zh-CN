...
简要地说，这是一个`String`（首字母“S”）对象包装形式，通常称为”原生“，与原始的字符串类型配对；它这是定义这个`toUpperCase()`方法在原型上的对象包装。

When you use a primitive value like `"hello world"` as an `object` by referencing a property or method (e.g., `a.toUpperCase()` in the previous snippet), JS automatically "boxes" the value to its object wrapper counterpart (hidden under the covers).
当你使用一个像“hello world”的原始值作为一个对象，通过参考一个在以前代码段中的属性或方法如`a.toUpperCase()`，JS自动“盒子”的值到其对象包装对应（隐藏在底层）。
...
（take care of翻译为处理）
一个字符串对象能够包装一个字符串值，一个Number对象能够包装一个数字，一个boolean对象能够包装一个布尔值。在大多数情况下，你不需要担心或直接使用这些对象包装器的形式的值 －－如果几乎所有的情况下都倾向于原始值形式，javascript将会替你处理剩余的部分。
...
强制转换在JavaScript中有两种形式：*显式转换* 和 *隐式转换*。显式强制转换只是你可以从代码中明显的看到从一种类型到另一种类型的转换将发生，而隐式强制转换是当类型转换时产生的不明显的副作用的结果。
