...
简要地说，这是一个`String`（首字母“S”）对象包装形式，通常称为”本地类型“，它包含了一个string类型的属性；它这是定义这个`toUpperCase()`方法在原型上的对象包装。

When you use a primitive value like `"hello world"` as an `object` by referencing a property or method (e.g., `a.toUpperCase()` in the previous snippet), JS automatically "boxes" the value to its object wrapper counterpart (hidden under the covers).
当你使用一个像“hello world”的原始值作为一个对象，通过参考一个在以前代码段中的属性或方法如`a.toUpperCase()`，JS会自动为这个值进行“装箱”（这是隐式发生的）。
...
（take care of翻译为处理）
'string'值会被包裹成'String'对象，'number'值会被包裹成'Number'对象，'boolean'值会被包裹成'Boolean'对象。在大多数情况下，你不需要担心或直接使用这些对象包装器的形式的值 －－如果几乎所有的情况下都倾向于原始值形式，javascript将会替你处理剩余的部分。
...
**注意：** 有关JS本地类型和“装箱”的更多信息，请看本系列*Types＆Grammar*标题的第3章。为了更好的理解对象的原型，请看本系列的*this＆Object Prototypes*标题的第5章。
...
我们在第1章中简要地讨论了强制转换，但让我们在这里重温一下。

Coercion comes in two forms in JavaScript: *explicit* and *implicit*. Explicit coercion is simply that you can see obviously from the code that a conversion from one type to another will occur, whereas implicit coercion is when the type conversion can happen as more of a non-obvious side effect of some other operation.
强制转换在JavaScript中有两种形式：*显式转换* 和 *隐式转换*。显式强制转换只是你可以从代码中明显的看到从一种类型到另一种类型的转换将发生，而隐式转换是当类型转换时产生的不明显的副作用的结果。
...
你可能已经听到了诸如“强制转换是邪恶”的情绪，这是因为有明显的地方，强制转换可以产生一些令人惊讶的结果。也许没有什么能让开发人员比起语言更能感到挫折的了。
...
强制转换不是邪恶的，也不必惊讶。事实上，大多数情况下，你可以用类型强制构造是相当明智和可理解的，甚至可以用于提高代码的可读性。我们不会进一步讨论这场辩论－－本系列的* Types＆Grammar *标题的第4章涵盖所有方面。
...
在第1章中，我们简要提到了值的“真”和“假”性质：当一个非布尔值被强制转换为一个布尔值时，它分别成为“真”或“假”吗？
...
重要的是要记住，一个非`boolean`值只有遵循这个“真值”/“假值”强制转换，如果它实际上强制转换为一个“布尔”。不要这样迷惑自己，当一个不是布尔值，而非要转成布尔值。
#### 相等(Equality)
...
在`a == b`比较中，JS注意到类型不匹配，所以它通过有序的一系列步骤来强制转换一个或两个值到不同的类型，直到类型匹配，然后一个简单的值相等检查。
...
如果你想到它，有两种可能的方式`a == b`可以通过强制转换给予`true`。比较可能最终为`"42" == "42"`，也可以是`"42"=="42"`。那是什么呢？
...
`a === b`产生`false`，因为强制转换是不允许的，所以简单的值比较显然失败了。许多开发人员认为`===`是更可预测的，因此他们主张始终使用该形式并远离`==`。我认为这个观点很短视。我相信`==`是一个强大的工具，可以帮助你的程序，*如果你花时间来了解它是如何工作的。
...
我们不会涵盖所有关于`==`比较中的强制转换如何在这里工作的细节细节。大部分是相当明智的，但有一些重要的角落要小心。您可以阅读ES5规范 (http://www.ecma-international.org/ecma-262/5.1/) 的第11.9.3节以查看确切的规则，你会惊讶于这种机制是多么简单，相比之下所有的负面炒作。
...
例如，`array`s在默认情况下通过用逗号（`，`）连接所有的值强制转换为`string`s。你可能会认为两个具有相同内容的数组是`==`等于，但是它们不是：
...
这里发生了什么？在ES5规范的第11.8.5节中，它说如果`<`比较中的两个值都是`string`s，因为它和`b <c`一样，比较是按字典顺序进行的（像字典一样）。但是如果一个或两个不是一个`字符串`，因为它与`a<b`，那么两个值都被强制转换为`数字`，并且发生典型的数字比较。
...
Metaphorically, this behavior is called *hoisting*, when a `var` declaration is conceptually "moved" to the top of its enclosing scope. Technically, this process is more accurately explained by how code is compiled, but we can skip over those details for now.当一个`var`声明在概念上“移动”到它的封闭范围的顶部
用比喻的说法，把这种想象叫做*提升*，从概念上讲，当一个`var`声明时，该作用域就会被“移动到”它的作用域的顶部。从技术上讲，这个过程更加准确的解释了如何编译代码，但是我们现在可以跳过这些细节。
...
你可以把闭包当成是"记住"和获取一个函数作用域(它的变量)的一种方法，即使该函数已经运行完毕。
Consider:
...
当我们调用 `makeAdder(1)`时，我们得到了一个它内部`add(...)`函数的引用,它记录了`x`变量1。我们将这个函数引用称为`plusOne(..)`
2. When we call `makeAdder(10)`, we get back another reference to its inner `add(..)` that remembers `x` as `10`. We call this function reference `plusTen(..)`.
当我们调用`makeAdder(10)`，我们得到了它另一个`add(...)`函数的引用，它记录了`x`变量10.我们将这个函数引用称为`plusTen(...)`
